...
func _valueErrorOutOfRange(v value, args ...interface{}) (fmtString string, fmtArgs []interface{}) {
	...
	// var minFmt string
	// if minIntf != nil {
	//   if _isOfType(minIntf, "int64") { minFmt = "%d" } else if _isOfType(minIntf, "float64") { minFmt = "%f" }
	...
	if min != nil {
		if max != nil {
			fmtString = v.String() + " is out of range <ansiSecondaryLiteral>[" + getFmt(min) + ", " + getFmt(max) + "]"
			fmtArgs = args
		} else {
			fmtString = v.String() + " is less then <ansiOutline>minLimit <ansiPrimaryLiteral>" + getFmt(min)
			fmtArgs = []interface{}{min}
		}
	} else if max != nil {
		fmtString = v.String() + " is greater then <ansiOutline>maxLimit <ansiPrimaryLiteral>" + getFmt(max)
		fmtArgs = []interface{}{max}
	}
	return
}

func getFmt(limit interface{}) (fmt string) {
	if _isOfType(limit, "int64") {
		fmt = "%d"
	} else if _isOfType(limit, "float64") {
		fmt = "%f"
	} else {
		bwerror.Panic("limit %#v is expected to be int64 or float64")
	}
	return
}
