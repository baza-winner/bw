...
_initBwProjCmd() {
  ...
  eval ${bwProjShortcut}'_docker_up() { eval "$_funcParams2"
    ...
    _pushd "$_'$bwProjShortcut'Dir/docker"
      ...
      local dockerComposeEnvFileSpec=".env"
      {
        ...
        echo "_containerName=$_'$bwProjShortcut'DockerContainerName"
        echo "_imageName=$_'$bwProjShortcut'DockerImageName"
      ...
      # export _hostIpList=( $(_getOwnIpList) )
      # export _hostIp=${_hostIpList[0]}

      # sudo ifconfig en0 alias 10.254.254.254 255.255.255.0

      if [[ "${#restart[@]}" -eq 0 ]]; then
        if [[ -n $forceRecreate ]]; then
          local -a OPT_forceRecreate=( --force-recreate )
        fi
        _dockerCompose up -d "${OPT_forceRecreate[@]}" --remove-orphans 2> >(tee "$stderrFileSpec"); local returnCode=$?
      else
    ...
    # if [[ "${#restart[@]}" -eq 0 ]]; then
    #   _docker exec -it "$_'$bwProjShortcut'DockerContainerName" bash
  ...
  eval '_'$bwProjShortcut'_dockerContanerNames() { eval "$_funcParams2"
    local -a containerNames=()
    local containerNameHolder; for containerNameHolder in $(cat docker-compose.yml | perl -ne "print if s/^\s*container_name:\s*(.+?)\s*/\$1/"); do
  ...
  eval ${bwProjShortcut}_docker_bashParams='( 
    '\''containerName:( $(_'$bwProjShortcut'_dockerContanerNames) )='\'\$_${bwProjShortcut}DockerContainerName'
  )'
